...
const Post = ({navigation}) => {
  ...
  const handlePost = async () => {
    try {
      ...
      const {data} = await axios.post("/post/create-post", {category, title, description})
      setLoading(false)
      alert(data?.message)
   ...
   return (
    ...
    <View style={{alignItems: "center"}}>
      ...
      <TextInput style={styles.inputBox} placeholder="add post description" placeholderTextColor={"gray"} multiline={true} numberOfLines={6} value={description} onChangeText={(text) => setDescription(text)} />
